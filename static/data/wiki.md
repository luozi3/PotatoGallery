...
- `tag`：自动识别为标签或关键词。
- `-tag`：排除标签。
- 标签匹配会忽略大小写与空格（如 `lilywhite` ≈ `lily white`）。若去空格后存在多标签歧义，则视为普通文本。
...
- 直接输入词语也会被当作文本匹配。
- 标签含空格时请使用引号：`tag:"lily white"` 或 `# "lily white"`（搜索提示点击会自动补引号）。
...
- 搜索框与标签输入都会提示可能的标签与别名，点击即可补全。
- 搜索框不需要 `#` 也会弹出候选，默认最多 10 条，别名优先。
- 别名提示会显示为 `别名 -> 主标签`，并自动补全为主标签。
- 发布时若缺少父标签，会提示补全父标签。
